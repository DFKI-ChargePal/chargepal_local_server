...
def read_data(table_name, battery_name, column_name)->Union[str, int]:
    sql = MySQLAccess()
...
def monitor_result(table_name:str, battery_name:str, column_name:str, expected_result:Union[str, int])->bool:
...
def wakeup(cart_name:str)->bool:
    ...
    state_battery_mode = read_data("CAN_MSG_RX_LIVE", cart_name, "State_bat_mod").lower()
...
def mode_req_standby(cart_name:str)->bool:
    ...
    if "standby" in state_battery_mode and "error" not in state_battery_mode:##verify how to know if standby is reached?
        success = True
    
    # if current state is BAT_ONLY -> proceed with request
    elif "error" not in state_battery_mode and mode_bat_only==1:
        publish_message(message_mode_req_standby)
        if check_feedback(cart_name, "Standby_OK"):
            success = monitor_result("CAN_MSG_RX_LIVE", cart_name, "Mode_Bat_only", 1) ##verify how to know if standby is reached?
...
def mode_req_idle(cart_name:str)->bool:
    ...
    # if current state is BAT_ONLY -> proceed with request
        ...
        publish_message(message_mode_req_idle)
        if check_feedback(cart_name, "Mode_request_idle_OK"):
            success = monitor_result("CAN_MSG_RX_LIVE", cart_name, "Flag_Modus", "Flag_idle")
...
def mode_req_EV_AC_Charge(cart_name: str)->bool:
    ...
    # if current state is IDLE -> proceed with request
        ...
        publish_message(message_mode_req_EV_AC_Charge)
        if check_feedback(cart_name, "EV_AC_Charge_OK"):
            success = monitor_result("CAN_MSG_RX_LIVE", cart_name, "Flag_Modus", "Flag_EV_AC_Charge")
...
def mode_req_EV_DC_Charge(cart_name:str)-> bool:
    ...
    # if current state is IDLE -> proceed with request
        ...
        publish_message(message_mode_req_EV_DC_Charge)
        if check_feedback(cart_name, "EV_DC_Charge_OK"):
            success = monitor_result("CAN_MSG_RX_LIVE", cart_name, "Flag_Modus", "Flag_EV_DC_Charge")
...
def mode_req_Bat_AC_Charge(cart_name:str)->bool:
    ...
    # if current state is IDLE -> proceed with request
        ...
        publish_message(message_mode_req_Bat_AC_Charge)
        if check_feedback(cart_name, "Bat_AC_Charge_OK"):
            success = monitor_result("CAN_MSG_RX_LIVE", cart_name, "Flag_Modus", "Flag_Bat_AC_Charge")
...
def ladeprozess_start(cart_name:str,station_name:str,charging_type:str)-> bool:
    flg_modus = read_data("CAN_MSG_RX_LIVE", cart_name, "Flag_Modus").lower()
    success = False
    if "flag_ev_ac_charge" in flg_modus or "flag_ev_dc_charge" in flg_modus or "flag_bat_ac_charge" in flg_modus:
        if "flag_ev_dc_charge" not in flg_modus: 
            message_unlock_request = "1793,2,64,0"  # not when EV_DC_Charge Ladeprozess Start
            publish_message(message_unlock_request)
        ...
        publish_message(message_plug_process_finished)
...
def monitor_plug_unlock(cart_name:str,station_name:str)->bool:
    if "ADS" in station_name:
        ...
        return monitor_result("CAN_MSG_RX_LIVE", cart_name, "AC_Charger_inlet_UNLOCKED", 1)
    
def ladeprozess_end(cart_name:str,station_name:str)->bool:
    flg_modus = read_data("CAN_MSG_RX_LIVE", cart_name, "Flag_Modus").lower()
    success = False
    ...
    if "flag_ev_ac_charge" in flg_modus or "flag_ev_dc_charge" in flg_modus or "flag_bat_ac_charge" in flg_modus:
        ...
        publish_message(message_mode_req_idle)
        if check_feedback(cart_name, "Mode_request_idle_OK"):
                if monitor_plug_unlock(cart_name,station_name):
                    message_plug_process_finished = "1793,2,0,1"
                    publish_message(message_plug_process_finished)
                    if check_feedback(cart_name, "PlugProcessFinished_Received"):
                        success = monitor_result("CAN_MSG_RX_LIVE", cart_name, "Mode_Bat_only", 1)
